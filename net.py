...
class AdaptationNet(object):
    ...
    def create_model(input_shape, num_classes, loss_function, metrics):
        model = AdaptationNet._alex_net(input_shape, num_classes)
...
# class RotationNet(object):
#     @staticmethod
#     def convert_model(model):
#         new = tf.keras.models.clone_model(model)
#         for layer in new.layers[:-2]:
#             layer.trainable = False
#
#         def l2_regularized_loss(y_true, y_pred):
#             loss = tf.keras.losses.mean_squared_error(y_true=y_true, y_pred=y_pred)
#             regularization_loss = tf.add_n([ tf.nn.l2_loss(v) for v in model.trainable_weights]) # doesnt converge
#             # regularization_loss = tf.add_n([ tf.nn.l2_loss(v) for v in model.layers[-2].weights])# didnt try yet
#             return loss + regularization_loss
#
#         new.compile(optimizer=Adam(), loss=l2_regularized_loss, metrics=model.metrics)
#         return new
